...
INC_FILES = $(wildcard $(INC_PATH)/*.hpp) $(wildcard $(INC_PATH)/*.h)
FILE_NAMES = $(sort $(notdir $(CPP_FILES:.cpp=)) $(notdir $(INC_FILES:.h=)))

# Define as regras para construir os arquivos .o
$(BIN_PATH)/%.o: $(DEP_PATH)/%.d | folders
	$(COMPILER) $(INC_PATHS) $(addprefix $(SRC_PATH)/,$(notdir $(<:.d=.cpp))) -c $(FLAGS) -o $@

# Define as regras para gerar os arquivos .d
$(DEP_PATH)/%.d: $(SRC_PATH)/%.cpp $(INC_FILES) | folders
...
release: FLAGS := $(filter-out $(WINDOWS_FLAGS),$(FLAGS))
release: $(EXEC)

debug: FLAGS += $(DFLAGS)
debug: FLAGS += $(DEBUG_WINDOWS_FLAGS)
debug: $(EXEC)
